...
// appelle des class
    ...
    while($data = $liste->fetch()) {
        if($data['mot_de_passe'] == '' and $data['bloquer'] == 0){
        ...
        }elseif($data['mot_de_passe'] == '' and $data['bloquer'] != 0 ){
            $status = ' <span class="tag tag-radus btn-red-transparent">Bloqué</span>';
            $bloquer = '<a href="javascript:void(0);" id="bDel" type="button" class="btn  btn-sm btn-transparence-purple" onclick="debloquer('.$data['id_admin'].')" style="padding: 3px 11px !important;">
                                            <i class="fa fa-lock" aria-hidden="true"></i>
                                        </a>';
        }elseif($data['mot_de_passe'] != '' and $data['bloquer'] != 0 ){
            $status = ' <span class="tag tag-radus tag tag-radus ">Bloqué</span>';
            $bloquer = '<a href="javascript:void(0);" id="bDel" type="button" class="btn  btn-sm btn-transparence-purple" onclick="debloquer('.$data['id_admin'].')" style="padding: 3px 11px !important;">
                                            <i class="fa fa-lock" aria-hidden="true"></i>
                                        </a>';        }else{
            $status = '<span class="tag tag-radus btn-green-transparent">Active</span>';
            $bloquer = '<a href="javascript:void(0);" id="bDel" type="button" class="btn  btn-transparence-info" onclick="bloquer('.$data['id_admin'].')">
                                            <i class="fa fa-unlock" aria-hidden="true"></i>
                                        </a>';
